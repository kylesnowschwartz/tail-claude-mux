...
> **Iteration 1.** Refinement of iteration-0. The bones (sparkline + gutter +
> small-multiples + demoted source) are preserved. The edges (sparkline
> contract, multi-source layout, verb dictionary, exception placement,
> codepoint widths, unfocus rules) are pinned.
...
The seed used eyebrow-only, which thrashes under multi-source interleave
(every row triggers a fresh eyebrow, eating 30–50% of the vertical budget).
Iteration 1 commits a **hybrid rule** keyed on run-length:
