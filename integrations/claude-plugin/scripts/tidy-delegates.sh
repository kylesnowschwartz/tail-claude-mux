...
# start command runs `codex --profile tcm-delegate` (set by this plugin's
# workflows/lib/tcm-spawn.sh). This never matches a codex shell you opened
# yourself, so the sweep is safe to run at any time.
#
